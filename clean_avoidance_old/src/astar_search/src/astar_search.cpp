...
AstarSearch::AstarSearch()
{
  ...
  private_nh_.param<double>("time_limit", time_limit_, 8000.0);

  // robot configs
  private_nh_.param<double>("robot_length", robot_length_, 0.5);
  private_nh_.param<double>("robot_width", robot_width_, 0.5);
  private_nh_.param<double>("robot_base2back", robot_base2back_, 0.5);
  private_nh_.param<double>("minimum_turning_radius", minimum_turning_radius_, 0.5);
...
{
  std::cout << "AstarSearch Fun is START !" << std::endl;
...
bool AstarSearch::setStartNode(const geometry_msgs::Pose& start_pose)
{
  ...
  {
    std::cout << "Check if start is valid !" << std::endl;
    return false;
  }
...
bool AstarSearch::setGoalNode(const geometry_msgs::Pose& goal_pose)
{
  ...
  {
    std::cout << "Set Goal Node :" << "isOutOfRange(index_x, index_y) || detectCollision(goal_sn)" << std::endl;
    return false;
  }
  ...
  if (use_wavefront_heuristic_)
  {
    ...
    if (!wavefront_result)
    {
      std::cout << "Set Goal Node :" << "!wavefront_result" << std::endl;
...
bool AstarSearch::isOutOfRange(int index_x, int index_y)
{
  ...
  {
    std::cout << "index_x : " << index_x << "index_y : " << index_y << "costmap_.info.width : " << static_cast<int>(costmap_.info.width)
                       << "costmap_.info.height : " << static_cast<int>(costmap_.info.height) << std::endl;
    return true;
  }
  return false;
}
...
bool AstarSearch::search()
{
  ...
  while (!openlist_.empty())
  {
    ...
    if (msec > time_limit_)
    {
      ...
      //TAG:
      std::cout << "Exceed time limit of " << time_limit_ << std::endl;
      return false;
    }
    ...
    for (const auto& state : state_update_table_[top_sn.index_theta])
    {
      ...
      {
        //std::cout << "AstarSearch::search(), Check if the index is valid" << std::endl;
        continue;
      }
  ...
  //TAG:
  std::cout << "Open list is empty ... Failed to find path ..." << std::endl;
  return false;
}
...
bool AstarSearch::detectCollision(const SimpleNode& sn)
{
  ...
  for (double x = left; x < right; x += resolution)
  {
    for (double y = top; y > bottom; y -= resolution)
    {
      ...
      if (isOutOfRange(index_x, index_y))
      {
        //std::cout << "detectCollision" << "isOutOfRange(index_x, index_y)" << std::endl;
        return true;
      }
      else if (nodes_[index_y][index_x][0].status == STATUS::OBS)
      {
        //std::cout << "detectCollision" << "nodes_[index_y][index_x][0].status == STATUS::OBS" << std::endl;
        return true;
      }
    }
  }
