...
PurePursuit::PurePursuit() : ld_(0.9), v_max_(0.5), v_(0.1), w_max_(1.0), pos_tol_(0.1), idx_(0),
                             goal_reached_(true), nh_private_("~"), tf_listener_(tf_buffer_),
                             map_frame_id_("map"), robot_frame_id_("base_link"),
                             lookahead_frame_id_("lookahead"),task_percentage_(0)
{
  // Get parameters from the parameter server
  nh_private_.param<double>("wheelbase", L_, 1.0);                                                       //轴距
  nh_private_.param<double>("lookahead_distance", ld_, 0.9);                                 //前沿点距离
...
float PurePursuit::ReadTaskPercentage()
{
  return task_percentage_;
}
  ...
  try
  {
    ...
    for (; idx_ < path_->poses.size(); idx_++)
    {
      if (Distance(path_->poses[idx_].pose.position, tf.transform.translation) > ld_)
      {
        ...
        int temp_idx = (idx_-ld_/0.1) > 0 ? (idx_-ld_/0.1) : 0;
        task_percentage_ = float(temp_idx)/float(path_->poses.size()-1);
        break;
      }
    }
    ...
    if (!path_->poses.empty() && idx_ >= path_->poses.size())
    {
      ...
      else
      {
        ...
        float min_index;
        for(int i = path_->poses.size()-21; i < path_->poses.size(); i++){
           float dist = Distance(path_->poses[i].pose.position, tf.transform.translation);
           if(dist > 0 && dist < min_dist){
              min_dist = dist;
              min_index = i;
            }
        }
        task_percentage_ = float(min_index)/float(path_->poses.size()-1);
      }
    }
