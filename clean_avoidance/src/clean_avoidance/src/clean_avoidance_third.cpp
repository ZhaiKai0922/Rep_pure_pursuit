...
class CleanAvoidance{
    ...
    int first_search_waypoints_delta_ = 30;     //2.7 - 1.0 = 1.7m
...
bool CleanAvoidance::CycleFixPath(){
    ...
    bool pause_flag = false;                                                        //纯路径跟踪线程中断标志位
        ...
        for(unsigned int j = x_min; j < x_max; j++){
            for(unsigned int k = y_min; k < y_max; k++){
                if(my_costmap_->getCost(j, k) == costmap_2d::LETHAL_OBSTACLE ||
                    ...
                    if(pub_vel_thread_flag_ == true && path_init_flag_ == true){
        ...
        if(avoid_flag_){
            ...
            for(int i = first_search_waypoints_delta_; i < search_waypoints_size_; i += search_waypoints_delta_){
                ...
                if(found_path && !global_plan.empty()){
                    ...
                    if(start_waypoint_index_ > 3){
                        start_waypoint_index_ = start_waypoint_index_ - 3;
                    }
                }
...
int main(int argc, char** argv){
    ros::init(argc, argv, "clean_avoidance_third_node");
